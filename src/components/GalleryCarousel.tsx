...
// 2. KARTA KOMPONENTI: Rasm manbasi to'g'rilandi
  ...
  return (
    ...
    >
      {card.isVideo ? (
        ...
        <Box
          component="img"
          src={card.image} // <-- XATOLIK SHU YERDA EDI, TO'G'RILANDI
...
// 3. ASOSIY KARUSEL KOMPONENTI: Avtomatik harakatlanish va yangi tugma pozitsiyalari
const GalleryCarousel = () => {
  const scrollRef = React.useRef<HTMLDivElement>(null);
  const [activeIndex, setActiveIndex] = React.useState(0);
  const [isHovering, setIsHovering] = React.useState(false);

  // Har 2 soniyada kartani almashtirish uchun avtomatik aylanish
  React.useEffect(() => {
    const interval = setInterval(() => {
      if (!isHovering) {
        setActiveIndex((prevIndex) => (prevIndex + 1) % cards.length);
      }
    }, 2000); // Tezlik 2 soniyaga o'zgartirildi

    return () => clearInterval(interval);
  }, [isHovering]);

  // activeIndex o'zgarganda karuselni aylantirish
  React.useEffect(() => {
    if (scrollRef.current) {
      const cardElement = scrollRef.current.children[0].children[
        activeIndex
      ] as HTMLElement;
      if (cardElement) {
  ...
  // Mobil uchun tugmalar yordamida aylantirish
  ...
  return (
    ...
    >
      ...
      {/* Mobil qurilmalar uchun markaziy yon tarafda joylashgan navigatsiya tugmalari */}
      <Box
        sx={{
          ...
          zIndex: 10,
          pointerEvents: 'none', // Faqat tugmalar bosilishi uchun
        }}
      >
        <IconButton
